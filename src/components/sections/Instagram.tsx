 ...
 * an hour, so the grid is part of the static HTML and costs the browser nothing.
 * Without a token it renders the bundled fallback images and is identical to
 * the design.
 */
    ...
    <section className="border-line-9 border-t">
      <Container className="phone:pt-14 phone:pb-16 py-[72px]">
        ...
        <p className="text-bone-35 mt-[18px] mb-0 font-mono text-[12px] tracking-[.06em]">
          {live
            ? `Live from the ${SITE.instagramHandle} feed, refreshed hourly.`
            : `Grid auto-syncs with the ${SITE.instagramHandle} feed once the Instagram token is connected.`}
        </p>
      </Container>
    </section>
  );
